...
# recall.
NAME_TYPES = ['first', 'last', 'patronym']
# , 'cause_of_death', 'last_extended', 'cause_of_death_extended']
DEBUG = True
WARNINGS = False
...
class NameNormalizer(object):
    ...
    def normalize_all(self, name):
        ...
        """
        parts = list(map(self.normalize, name.split()))
        return ' '.join(parts)
    ...
    def _construct_trie(self):
        node0 = TrieNode()
        n_nodes = 1
        for name, (norm_name, name_popularity, _, _) in self.name_map.items():
